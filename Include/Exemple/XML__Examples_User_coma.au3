;~ https://www.autoitscript.com/forum/topic/183128-multiple-attribute-parsing-with-xmldomwrapper/
...
Func _Tasks_XML()
	...
	Local $s_XML = ''
	$s_XML &= '<body>' & @CRLF
	$s_XML &= '    <net name="Foo" ipv=" 4" ip=" 192.168.1.0">' & @CRLF
	$s_XML &= '        <node ip="192.168.1.170" type=" Gateway">gw</node>' & @CRLF
	$s_XML &= '        <node ip="192.168.1.171" type=" Rpi">rpi</node>' & @CRLF
	$s_XML &= '    </net>' & @CRLF
	$s_XML &= '    <net name="Bar" ipv=" 4" ip=" 192.168.2.0">' & @CRLF
	$s_XML &= '        <node ip="192.168.2.172" type=" Gateway">gw</node>' & @CRLF
	$s_XML &= '        <node ip="192.168.2.173" type=" Rpi">rpi</node>' & @CRLF
	$s_XML &= '    </net>' & @CRLF
	$s_XML &= '</body>' & @CRLF

	; load XML data to the object
	_XML_LoadXML($oXMLDoc, $s_XML)

	; Tidy XML
	_XML_LoadXML($oXMLDoc, _XML_TIDY($oXMLDoc))

	; selecting nodes
	Local $oNodesColl = _XML_SelectNodes($oXMLDoc, "//net")
	If @error Then
	...
	Local $aAttributes = _XML_GetAllAttrib($oXMLDoc, "//net[1]")
	If @error Then
