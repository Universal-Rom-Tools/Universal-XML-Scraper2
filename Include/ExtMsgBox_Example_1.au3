...
While 1
	...
	Switch GUIGetMsg()
		Case $GUI_EVENT_CLOSE, $hButton7
			...
			_ExtMsgBoxSet(1)
			$nTest = _ExtMsgBox(32, "Yes|&No", "Query", "Are you sure?", 0, $hTest_GUI, 0, False)
		...
		Case $hButton4
			...
			$sMsg &= "There are some very long lines, so the message box width is set to the default maximum and the text will be forced to wrap as it is much too long to fit" & @CRLF & @CRLF
			...
			$iRetValue = _ExtMsgBox ($EMB_ICONSTOP, "And the button will expand to fit its text", "Test 4", $sMsg, 15)
